...
function _reinitializePluginConfigs() {
  _reinitializePluginConfigs = _asyncToGenerator(function* (options) {
    ...
    var localChecksums = yield (0, _pluginChecksumManifest.getChecksums)(workDir);
    var allPlugins = yield (0, _resolver.default)({
      basePath: workDir
    });
    var pluginsWithDistConfig = (yield Promise.all(allPlugins.map(pluginHasDistConfig))).filter(Boolean);
    var distChecksums = yield Promise.all(pluginsWithDistConfig.map(getPluginConfigChecksum));
    var withLocalConfigs = yield Promise.all(distChecksums.map(hasLocalConfig));
    var missingConfigs = yield Promise.all(withLocalConfigs.map(createMissingConfig));
    var configPlugins = missingConfigs.map(warnOnDifferingChecksum);
    ...
    function createMissingConfig(plugin) {
      ...
      var srcPath = _path.default.join(plugin.path, 'config.dist.json');

      var dstPath = _path.default.join(workDir, 'config', "".concat((0, _normalizePluginName.default)(plugin.name), ".json"));

      var prtPath = _path.default.relative(workDir, dstPath);

      if (!flags.quiet) {
        output.print("Plugin \"".concat(plugin.name, "\" is missing local configuration file, creating ").concat(prtPath));
      }
    ...
    function saveNewChecksums(plugins) {
      var sums = Object.assign({}, localChecksums);
...
function _tryInitializePluginConfigs() {
  _tryInitializePluginConfigs = _asyncToGenerator(function* (options) {
    ...
    try {
      ...
      var manifest = yield _fsExtra.default.readJson(_path.default.join(options.workDir, 'package.json')).catch(() => ({}));
      var dependencies = Object.keys(Object.assign({}, manifest.dependencies, manifest.devDependencies));
      var depName = err.plugin[0] === '@' ? err.plugin : "sanity-plugin-".concat(err.plugin);

      if (dependencies.includes(depName)) {
        err.message = "".concat(err.message, "\n\nTry running \"sanity install\"?");
      } else {
        err.message = "".concat(err.message, "\n\nTry running \"sanity install ").concat(depName, "\"?");
      }
...
function pluginHasDistConfig(plugin) {
  var configPath = getPluginConfigPath(plugin);
