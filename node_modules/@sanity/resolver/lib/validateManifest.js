...
function validateManifest(manifest, plugin) {
  ...
  (manifest.parts || []).forEach((part, i) => {
    var baseError = `Part defined at index ${i} of plugin "${plugin}" is invalid`;
...
function validatePartName(name, baseError) {
  var examples = ['Examples:', '- part:package-name/part-name', '- part:package-name/part-name-style', '', `See ${(0, _generateHelpUrl.default)('part-name-format')}`].join('\n');
