...
function mergeResult(plugins) {
  ...
  var partPlugins = plugins.map(plugin => {
    ...
    if (!Array.isArray(plugin.manifest.parts)) {
      var help = `See ${(0, _generateHelpUrl.default)('plugin-parts-syntax')}`;
...
function assignPartImplementation(plugin, part, implementations, definitions, options) {
  ...
  if (prevDefinition && !prevDefinition.isAbstract) {
    ...
    throw new Error(`${`Plugin ${_current} tried to implement part "${partName}", which is already declared` + ` as a non-overridable part by ${existing} - ` + 'See '}${(0, _generateHelpUrl.default)('implement-non-overridable-part')}`);
