...
describe('Product Variant Selector', () => {
	const renderDefault = () =>
		render(<ProductVariantSelector variants={variants} currentVariant={mockVariants[0]} selectVariant={noop} />)
	...
	it('should render nothing if there is only one variant', () => {
		const { container } = render(
			<ProductVariantSelector variants={[currentVariant]} currentVariant={currentVariant} selectVariant={selectVariant} />,
		)
